...
from src.globals import get_tt, get_afc, get_k_pv_dict
...
def find_trains(nid1: int, nid2: int, ts1: int, ts2: int, line: int, upd: int) -> list[tuple[int, int, int]]:
    ...
    # check if train_id is the same for consecutive rows
    train_ids = tt[:-1, 0] == tt[1:, 0]
    ...
    trains = [(tt[i, 0], tt[i, 5], tt[i + 1, 4])
              for i in range(len(tt) - 1) if condition[i]]
    return trains
...
def find_seg_trains(pv: np.ndarray, ts1: int, ts2: int) -> list[list[tuple[int, int, int]]]:
    ...
    for seg in pv:
        trains = find_trains(
            nid1=seg[1], nid2=seg[2], ts1=ts1_this_seg, ts2=ts2, line=seg[3], upd=seg[4])
...
def find_feas_iti(k_pv: np.ndarray, ts1: int, ts2: int) -> list[list[int | tuple[int, int, int]]]:
    ...
    for path_id in set(k_pv[:, 0]):
        ...
        seg_trains: list[list[tuple[int, int, int]]
                         ] = find_seg_trains(pv=pv, ts1=ts1, ts2=ts2)
        ...
        def backtrack(seg_idx: int, current_path: list[tuple[int, int, int]]):
            ...
            for train in seg_trains[seg_idx]:
                # check transfer time > 0
                if not current_path or current_path[-1][2] < train[1]:
...
def plot_seg_trains(k_pv: np.ndarray, ts1: int, ts2: int):
    ...
    # Ensure enough rows for all subplots
    nrows = (num_plots + 1) // ncols if num_plots > 4 else num_plots
    ...
    xticklabels = [ts2tstr(ts1, include_seconds=True)] + \
        xticklabels + [ts2tstr(ts2, include_seconds=True)]
    ...
    for i, (path_id, seg_trains) in enumerate(path_id_seg_trains.items()):
        ...
        for seg_id, trains in enumerate(seg_trains):
            line = pv[seg_id, 3]
            for train in trains:
                ax.plot([train[1], train[2]], [seg_id, seg_id + 1],
                        lw=1, color=line_colors.get(line, 'black'))
...
def find_feas_iti_all(save_feas_iti: bool = True, save_afc_no_iti: bool = True) -> pd.DataFrame:
    ...
    for rid, uid1, ts1, uid2, ts2 in tqdm(afc, total=afc.shape[0], desc="Finding feasible itineraries"):
        ...
        for iti_id, itinerary in enumerate(iti_list, start=1):
            ...
            for seg_id, (train_id, board_ts, alight_ts) in enumerate(itinerary[1:], start=1):
                data.append([rid, iti_id, path_id, seg_id,
                            train_id, board_ts, alight_ts])
    ...
    def _save_rids_not_found() -> pd.DataFrame | None:
        ...
        print(
            f"Not found feasible itinerary: {len(rids_not_found)} passengers.")
    ...
    if save_afc_no_iti:  # save rids without feasible itineraries
        ...
        if iti_not_found is not None:
            save_(fn=config.CONFIG["results"]["AFC_no_iti"],
                  data=iti_not_found, auto_index_on=False)
    ...
    if save_feas_iti:
        save_(fn=config.CONFIG["results"]["feas_iti"],
              data=df, auto_index_on=False)
...
def process_afc_chunk(chunk: np.ndarray, k_pv_dict: dict) -> list[list]:
    ...
    for rid, uid1, ts1, uid2, ts2 in chunk:
        ...
        for iti_id, itinerary in enumerate(iti_list, start=1):
            path_id = itinerary[0]
            for seg_id, (train_id, board_ts, alight_ts) in enumerate(itinerary[1:], start=1):
                results.append([rid, iti_id, path_id, seg_id,
                               train_id, board_ts, alight_ts])
    return results
...
def find_feas_iti_all_parallel(save_feas_iti: bool = True, save_afc_no_iti: bool = True,
    ...
    print(
        f"[INFO] Start finding feasible itineraries using {n_jobs} threads with chunk size {chunk_size}...")
    ...
    def _save_rids_not_found() -> pd.DataFrame | None:
        ...
        if rids_not_found.shape[0] > 0:
            print(
                f"[INFO] Not found feasible itinerary for {rids_not_found.shape[0]} passengers.")
    ...
    if save_afc_no_iti:
        ...
        if df_nf is not None:
            save_(fn=config.CONFIG["results"]["AFC_no_iti"],
                  data=df_nf, auto_index_on=False)

    # Convert to final dataframe
    df = pd.DataFrame(
        data,
        columns=['rid', 'iti_id', 'path_id', 'seg_id',
                 'train_id', 'board_ts', 'alight_ts']
    ).astype({
        'rid': 'int32',
        'iti_id': 'int32',
...
def _plot_check_feas_iti(rid: int = None, print_on:bool=True):
    ...
    if rid is None:
        rid, uid1, ts1, uid2, ts2 = afc[np.random.choice(
            len(afc))].flatten().tolist()
    else:
